...
function ScoreBadge({ score }: { score: number }) {
  ...
  return (
    <Badge className="bg-gray-100 text-gray-600 font-bold text-lg px-4 py-1.5">
      {score}점 · 보통
    </Badge>
  )
}
...
function RecommendationsContent() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const [matchRes, seniorRes] = await Promise.all([
        supabase
          .from("matches")
      ...
      if (matchRes.error) setFetchError(matchRes.error.message)
      else setMatches((matchRes.data ?? []) as unknown as MatchWithJob[])
  ...
  const pageTitle = seniorName ? `${seniorName} 님께 맞는 일자리` : "추천 일자리 목록"
    ...
    return (
      <div className="space-y-8">
        <div className="space-y-2">
          ...
          <p className="text-xl text-gray-500">내 프로필과 가장 잘 맞는 일자리를 점수 순으로 보여드립니다.</p>
        </div>
        <div className="bg-gray-50 border-2 border-gray-300 rounded-xl p-12 text-center space-y-4">
          ...
          <p className="text-xl text-gray-500">담당자가 직접 연락드리니 잠시만 기다려 주세요.</p>
        </div>
      </div>
    )
  }
