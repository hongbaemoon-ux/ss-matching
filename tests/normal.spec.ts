...
test.describe('정상 시나리오: 시니어 등록 → 자동 매칭 → 7점 카드', () => {
  test.beforeEach(async () => {
    await resetDb()
    // 서울/경비/required_career=3 공고 — senior(서울/경비/career=5)와 완벽 매칭
    // 직종+3 / 경력+2 / 지역(서울↔서울, 0km≤30km)+2 = 7점
    await insertJob({ title: '서울 경비원 모집', region: '서울', job_type: '경비', required_career: 3 })
  })

  test('등록 완료 메시지 표시 및 추천 페이지에 7점 금색 배지 카드가 상단에 표시된다', async ({ page }) => {
    ...
    // 지역 선택 (시/도)
    await page.getByText('시/도를 선택하세요').click()
    ...
    // 7점 금색 배지(⭐) 카드가 표시되는지 확인
    const goldBadge = page.getByText('7점')
